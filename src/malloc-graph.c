...
static Event *next_event(MallocGraph *g, const char *name) {
    Event *event = g->cursor;
    if (name && event->type == EV_CALL && !strcmp(event->scope->name, name)) return event;
    for (event = event->next; event && event->type == EV_CALL; event = event->next)
        if (name && !strcmp(event->scope->name, name)) return event;
...
static bool event(MallocGraph *g, EventType type, size_t value, size_t bytes, Event *scope) {
    ...
    if (g->recording) {
        ...
        e = next_event(g, type == EV_CALL ? scope->name : NULL);
        if (!e || e->type != type ||
...
static Event *find_scope(Event *event, const char *name) {
    for (; event; event = event->next) {
        if (event->type != EV_CALL) continue;
        if (!strcmp(event->scope->name, name)) return event->scope;
        Event *scope = find_scope(event->scope->next, name);
...
SHARED_EXPORT bool malloc_graph_push(void *handle, const char *name) {
    ...
    Event *scope = find_scope(g->root.next, name);
    ...
    bool repeat = g->recording && g->cursor->type == EV_CALL && g->cursor->scope == scope;
    if (!repeat && !event(g, EV_CALL, 0, 0, scope)) {
        if (recording) { free(scope->name); free(scope); }
        return false;
    }
    if (!push_stack(g)) {
        g->failed = true;
...
SHARED_EXPORT bool malloc_graph_pop(void *handle) {
    ...
    if (!g->recording && next_event(g, NULL)) { g->failed = true; return false; }
    if (g->stack) {
        State *state = g->stack;
        g->stack = state->next;
        g->scope = state->scope;
...
static void free_events(Event *event, Event **scopes) {
    while (event) {
        Event *next = event->next;
        if (event->type == EV_CALL && event->scope->name) {
            Event *scope = event->scope;
            char *name = scope->name;
            scope->name = NULL;
            free_events(scope->next, scopes);
            free(name);
            scope->next = *scopes;
            *scopes = scope;
        }
...
SHARED_EXPORT void malloc_graph_destroy(void *handle) {
    ...
    Event *scopes = NULL;
    free_events(g->root.next, &scopes);
    Event *scope = scopes;
    while (scope) {
        Event *next = scope->next;
        free(scope);
        scope = next;
    }
